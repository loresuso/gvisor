...
// forwardPacketWrite emits the pkt using the provided route.
//
...
func (e *endpoint) forwardPacketWrite(route *stack.Route, pkt *stack.PacketBuffer, updateOptions bool) ip.ForwardingError {
	h := header.IPv4(pkt.NetworkHeader().View())
...
func (e *endpoint) forwardUnicastPacket(pkt *stack.PacketBuffer) ip.ForwardingError {
	...
	return e.forwardPacketWrite(r, pkt, false /* updateOptions */)
}
...
func (e *endpoint) forwardMulticastPacket(h header.IPv4, pkt *stack.PacketBuffer) ip.ForwardingError {
	...
	routeKey := multicast.RouteKey{
		UnicastSource:        h.SourceAddress(),
		MulticastDestination: h.DestinationAddress(),
	}
	...
	case multicast.PendingRouteStateInstalled:
		if mcastDisp := e.protocol.options.MulticastForwardingDisp; mcastDisp != nil {
			mcastDisp.OnMissingRoute(stack.MulticastPacketContext{
...
func (e *endpoint) forwardValidatedMulticastPacket(pkt *stack.PacketBuffer, installedRoute *multicast.InstalledRoute) ip.ForwardingError {
	...
	if e.nic.ID() != installedRoute.ExpectedInputInterface() {
		if mcastDisp := e.protocol.options.MulticastForwardingDisp; mcastDisp != nil {
			h := header.IPv4(pkt.NetworkHeader().View())
			mcastDisp.OnUnexpectedInputInterface(stack.MulticastPacketContext{
				stack.UnicastSourceAndMulticastDestination{h.SourceAddress(), h.DestinationAddress()},
				e.nic.ID(),
			}, installedRoute.ExpectedInputInterface())
		}
		return &ip.ErrUnexpectedInputInterface{}
	}
...
//
// This method should be invoked by the endpoint that received the pkt.
func (e *endpoint) forwardMulticastPacketForOutgoingInterface(pkt *stack.PacketBuffer, outgoingInterface multicast.OutgoingInterface) ip.ForwardingError {
	h := header.IPv4(pkt.NetworkHeader().View())
	...
	//
	// Copying of the packet is deferred to forwardPacketWrite since unicast and
	// multicast both require a copy.
	if outgoingInterface.MinTTL > h.TTL() {
		return &ip.ErrTTLExceeded{}
	}

	route := e.protocol.stack.NewMulticastRoute(outgoingInterface.ID, h.DestinationAddress(), e.NetworkProtocolNumber())
	...
	return e.forwardPacketWrite(route, pkt, true /* updateOptions */)
}
...
func (e *endpoint) handleValidatedPacket(h header.IPv4, pkt *stack.PacketBuffer, inNICName string) {
	...
	// or it should be forwarded (if enabled).
...
func (e *endpoint) handleForwardingError(err ip.ForwardingError) {
	...
	case *ip.ErrNoBufferSpace:
		stats.Forwarding.NoBufferSpace.Increment()
	case *ip.ErrUnexpectedInputInterface:
		stats.Forwarding.UnexpectedInputInterface.Increment()
	case *ip.ErrUnknownEndpoint:
		stats.Forwarding.UnknownEndpoint.Increment()
	default:
...
func (p *protocol) newMulticastRouteKey(addresses stack.UnicastSourceAndMulticastDestination) (multicast.RouteKey, tcpip.Error) {
	if !p.isUnicastAddress(addresses.Source) || header.IsV4LinkLocalUnicastAddress(addresses.Source) {
		return multicast.RouteKey{}, &tcpip.ErrBadAddress{}
	}
	...
	return multicast.RouteKey{UnicastSource: addresses.Source, MulticastDestination: addresses.Destination}, nil
}

func (p *protocol) newMulticastRoute(route stack.MulticastRoute) (*multicast.InstalledRoute, tcpip.Error) {
	...
	for _, outgoingInterface := range route.OutgoingInterfaces {
		if route.ExpectedInputInterface == outgoingInterface.ID {
			return nil, &tcpip.ErrInputCannotBeOutput{}
		}
	...
	return p.multicastRouteTable.NewInstalledRoute(route.ExpectedInputInterface, route.OutgoingInterfaces), nil
}
...
func (p *protocol) AddMulticastRoute(addresses stack.UnicastSourceAndMulticastDestination, route stack.MulticastRoute) tcpip.Error {
	key, err := p.newMulticastRouteKey(addresses)
	if err != nil {
		return err
	}

	installedRoute, err := p.newMulticastRoute(route)
	if err != nil {
		return err
	}

	pendingPackets := p.multicastRouteTable.AddInstalledRoute(key, installedRoute)
...
// DisableMulticastCleanupRoutineForTesting implements
// stack.MulticastForwardingNetworkProtocol.
func (p *protocol) DisableMulticastCleanupRoutineForTesting() {
	p.multicastRouteTable.DisableCleanupRoutineForTesting()
}
...
func (p *protocol) isUnicastAddress(addr tcpip.Address) bool {
	...
	if ep := p.findEndpointWithAddress(addr); ep != nil {
		return false
	}
