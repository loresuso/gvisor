...
func (e *endpoint) forwardUnicastPacket(pkt *stack.PacketBuffer) ip.ForwardingError {
	...
	return e.forwardPacketWrite(r, pkt)
}
...
func (e *endpoint) forwardPacketWrite(route *stack.Route, pkt *stack.PacketBuffer) ip.ForwardingError {
	h := header.IPv6(pkt.NetworkHeader().View())
	stk := e.protocol.stack
...
func (e *endpoint) forwardMulticastPacketForOutgoingInterface(pkt *stack.PacketBuffer, outgoingInterface multicast.OutgoingInterface) ip.ForwardingError {
	...
	defer route.Release()
	return e.forwardPacketWrite(route, pkt)
}
