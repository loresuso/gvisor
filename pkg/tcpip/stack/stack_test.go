...
// MulticastForwardingNetworkProtocol.GetMulticastRouteLastUsedTime.
func (*fakeNetworkProtocol) GetMulticastRouteLastUsedTime(addresses stack.UnicastSourceAndMulticastDestination) (tcpip.MonotonicTime, tcpip.Error) {
...
func TestGetMulticastRouteLastUsedTime(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := s.GetMulticastRouteLastUsedTime(test.netProto, addresses)

			if !cmp.Equal(err, test.wantErr, cmpopts.EquateErrors()) {
				t.Errorf("got s.GetMulticastRouteLastUsedTime(%d, %#v) = %s, want %s", test.netProto, addresses, err, test.wantErr)
			}
		})
	}
}
