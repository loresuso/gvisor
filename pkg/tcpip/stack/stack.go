...
// addresses and protocol was used or updated.
func (s *Stack) GetMulticastRouteLastUsedTime(protocol tcpip.NetworkProtocolNumber, addresses UnicastSourceAndMulticastDestination) (tcpip.MonotonicTime, tcpip.Error) {
	netProto, ok := s.networkProtocols[protocol]
	if !ok {
	...
	return forwardingNetProto.GetMulticastRouteLastUsedTime(addresses)
}
...
// NewMulticastRoute returns a Route that may be used to forward multicast
// packets.
//
// Returns nil if validation fails.
func (s *Stack) NewMulticastRoute(nicID tcpip.NICID, remoteAddr tcpip.Address, netProto tcpip.NetworkProtocolNumber) *Route {
	nic, ok := s.nics[nicID]
	if !ok || !nic.Enabled() {
		return nil
	}
